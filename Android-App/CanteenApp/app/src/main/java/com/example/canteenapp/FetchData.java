...
public class FetchData extends AsyncTask<Void, Void, Void> {
    ...
    String BaseURL = "http://192.168.1.101:8070/";

    FetchData(CategoryFragment fragment) {
        this.categoryFragment = fragment;
    }

    FetchData(MenuFragment fragment) {
        this.menuFragment = fragment;
    }

    FetchData(OrderFragment fragment) {
    ...
    private void fetchMenu() {
        try {
            ...
            for (int i=0; i < items.length(); i++) {
                ...
                menuItem.setPrice((double) item.get("price"));
